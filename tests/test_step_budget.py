...
def test_budget_containment_allowed_after_min_step_until_report_deadline():
    early = budget_state(4, 15, 14, 5)
    middle = budget_state(6, 15, 14, 5)
    late = budget_state(13, 15, 14, 5)
    deadline = budget_state(14, 15, 14, 5)

    assert early.containment_allowed is False
    assert middle.containment_allowed is True
    assert late.containment_allowed is True
    assert deadline.containment_allowed is False
